...
if (system.args.length > 1 && system.args[1].toLowerCase().length > 1) {
	if(system.args[1].toLowerCase().indexOf("n/a") >= 0)
	{
		address = "https://www.youtube.com/results?search_query=Monstercat+Live+Stream";
	}
	else {
		address = "https://www.youtube.com/results?search_query=" + system.args[1].toLowerCase().replace(new RegExp(" ", 'g'), "+");
	}
}
else {
	address = "https://www.youtube.com/results?search_query=Monstercat+Live+Stream";
